var Skeleton = (function (exports) {
  ...
  const addBlick = () => {
    ...
    load.classList.add('blink');
    appEl.appendChild(load);
  ...
  function traverse(options) {
    ...
    const excludesEle = (excludes && excludes.length) ? Array.from($$(excludes.join(','))) : [];
    const grayEle = (grayBlock && grayBlock.length) ? Array.from($$(grayBlock.join(','))) : [];
